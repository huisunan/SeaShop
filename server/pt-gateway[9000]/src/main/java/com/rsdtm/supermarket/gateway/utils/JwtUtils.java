...
public class JwtUtils {
    ...
    public static String generateToken(User userInfo, PrivateKey privateKey, int expireMinutes) throws Exception {
        return Jwts.builder()
                ...
                .claim(JwtConstans.JWT_KEY_ROLES, userInfo.getAuthorities())
...
//    public static Date getCreatedDateFromToken(String token, PublicKey publicKey){
//
//    }
    /**
     * 公钥解析token
    ...
    public static boolean isTokenExpired(String token, byte[] publicKey) throws Exception {
        Date expirationDateFromToken = getExpirationDateFromToken(token, publicKey);
        return expirationDateFromToken.before(new Date());
    }

    public static Date getExpirationDateFromToken(String token, byte[] publicKey) throws Exception{
    ...
    public static User getInfoFromToken(String token, PublicKey publicKey)  {
        Jws<Claims> claimsJws = parserToken(token, publicKey);
        Claims body = claimsJws.getBody();
        List<?> roles = (List<?>) body.get(JwtConstans.JWT_KEY_ROLES);
        return new User(
                ObjectUtils.toInt(body.get(JwtConstans.JWT_KEY_ID)),
                ObjectUtils.toString(body.get(JwtConstans.JWT_KEY_USER_NAME)),
                ObjectUtils.toList(roles)
        );
    }
