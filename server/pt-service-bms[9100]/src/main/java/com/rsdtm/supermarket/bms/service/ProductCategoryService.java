...
public class ProductCategoryService {
    ...
    private int deleteChildren(Integer id){
        if (!categoryDao.existsWithPrimaryKey(id))
    ...
    public int updateProductCategory(BmsProductCategory productCategory) {
        ...
        if(!categoryDao.existsWithPrimaryKey(productCategory.getId()))
            throw new PtException(ExceptionEnum.CATEGORY_NOT_EXIST);
    ...
    public List<ProductCategoryVo> findListByParentId(Integer id){
        ...
        for (BmsProductCategory productCategory : select) {
            ...
            List<ProductCategoryVo> productCategoryVos =  findListByParentId(productCategory.getId());
            if (productCategoryVos.size() == 0)
                productCategoryVos = null;
            tempVo.setChildrenCategories(productCategoryVos);
            list.add(tempVo);
        }
        return list;
    }
